                  ...
                  <div ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps}
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '8px' }}>
                      ...
                      <button
                        ...
                        style={{ marginTop: '2px', width: '16px', height: '16px', borderRadius: '50%', border: '2px solid ' + (task.status === 'done' ? '#10b981' : (lockedTaskIds?.has(task.id) || task.is_locked) ? '#e5e7eb' : '#d1d5db'), background: task.status === 'done' ? '#10b981' : 'transparent', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontSize: '10px', lineHeight: 1 }}
                      >
                        {task.status === 'done' && '\u2713'}
                      </button>
                      )}
                      <div style={{ flex: 1 }}>
                        <p style={{ fontSize: '14px', color: task.status === 'done' ? '#9ca3af' : (lockedTaskIds?.has(task.id) || task.is_locked) ? '#9ca3af' : '#1f2937', margin: 0, textDecoration: task.status === 'done' ? 'line-through' : 'none' }}>
...
function MobileArtifacts({ user }) {
  const [artifacts, setArtifacts] = useState([])
  const [versions, setVersions] = useState({}) // artifactId -> array
  const [notifications, setNotifications] = useState({}) // artifactId -> count
  ...
  const [extracting, setExtracting] = useState(null) // { artifact, version }
...
export default function MobileLayout({
  ...
  const [aaSubTab, setAaSubTab] = useState('assistant') // 'assistant' | 'artifacts'
