...
export default function App() {
  ...
  const fetchTasks = useCallback(async () => {
    ...
    const [tasksRes, goalsRes, goalTasksRes] = await Promise.all([
      supabase.from('tasks').select('*').order('created_at', { ascending: false }).limit(1000000),
      supabase.from('goals').select('*').order('created_at'),
      supabase.from('tasks').select('id, title, goal_id, status, due_date, start_time, priority, collaboration_id, assigned_to').not('goal_id', 'is', null).limit(1000000)
    ])
            ...
            <div className="flex items-center gap-2">
              {(calView === 'week' || calView === 'workweek') && <>
                ...
                <span className="text-sm font-medium text-gray-700 min-w-[200px] text-center">
                  {calView === 'workweek'
                    ? `${format(addDays(weekStart, 1), 'MMM d')} - ${format(addDays(weekStart, 5), 'MMM d, yyyy')}`
